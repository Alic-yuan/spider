...
if __name__ == '__main__':
    ...
    place = TreeLevel1({'db': 'fsyd', 'wd': 'reg', 'm': 'findZbXl'})
    places = set(place)
    tids = TreeLevel2({'db': 'fsyd', 'wd': 'zb', 'm': 'findZbXl'})
    for place in places:
        for id in tids:
            a = '[{"wdcode":"reg","valuecode":"%s"}]' % place
            b = '[{"wdcode":"zb","valuecode":"%s"}]' % id
            data = {'m': 'QueryData', 'dbcode': 'fsyd', 'rowcode': 'zb', 'colcode': 'sj',
                    'wds': a,
                    'dfwds': b}
            get(url,data)
