...
MONGO_TABLE = 'niandu_data'

client = MongoClient(MONGO_URL,connect=True)
db = client[MONGO_DB]

headers = {
'Cookie':'JSESSIONID=kPrshhQLZvRr967Hpky7rwLBW122TQNdVw19vnGBM2QYlchtB1pj!-933055557',
...
def TreeLevel(initDict, follow=True):
    ...
    for record in res1.json():
        try:
            ...
            data = {'m': 'QueryData', 'dbcode': 'hgnd', 'rowcode': 'zb', 'colcode': 'sj', 'wds': '[]',
                    'dfwds': a}
            get(url, data)
            if follow == True:
...
if __name__ == '__main__':
    TreeLevel({'db': 'hgnd', 'wd': 'zb', 'm': 'findZbXl'})
