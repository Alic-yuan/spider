...
class download():
    def __init__(self):
        ...
        self.iplist = []
        html = requests.get("http://www.66ip.cn/areaindex_4/1.html")
        a = BeautifulSoup(html.text, 'lxml')
        iplistn = a.find('div', id='main').find('div', id='footer').find('div', align='center').find('table').find_all(
            'tr')
        for ip in iplistn:
            b = ip.find_all('td')[0]
            i = b.get_text()
